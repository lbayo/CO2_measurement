...
import json
import st7789
from tft_lib import ini_tft, tft_display,text
...
'VersionDate' : '27-05-2021',                   # fecha version del programa
...
'ssidx'       : [['MIWIFI_2G_kYXg','Lu1sB4y00rt0n0v35']
                ],       
...
'displaytype' : 'I2C',                          # SPI o I2C (if SPI fixed pinout)
#'displaytype' : 'I2C',                          # SPI o I2C (if SPI fixed pinout)
#'displaytype' : 'SPI',                          # SPI o I2C (if SPI fixed pinout)
'SCL'         :   22,                           # I²C Pin scl
'SDA'         :   21,                           # I²C Pin sda
'SPI_SCK'     :   19,
'SPI_MOSI'    :   21,
...
'SPI_RES'     :    5,
'SPI_CS'      :   16,
'sensorCO2'   : 'CCS811',                       # TIPO: CCS811 o MH-Z19B
#'sensorCO2'   : 'MH-Z19B',                      # TIPO: CCS811 o MH-Z19B
...
#'TEMPERATURA' :   26,                           # pin lector temperatura DHT-11 o DHT-22
'TEMPERATURA' :   33,                           # pin lector temperatura DHT-11 o DHT-22
'TIPOLECTOR'  : "DHT11",                        # tipo de lector de temperatura
#'TIPOLECTOR'  : "DHT22",                       # tipo de lector de temperatura
'mqtt'        : True,                           # enable MQTT protocol
'factor_cal'  : [0,1],                          # ajuste emdidas de CCS811 a MH-Z19B medida= a+b*lectura
'server'      : 'test.mosquitto.org',               # IP address or URL for MQTT broker
'port'        : 1883,
'qos'         :    0,                           # mqtt quality of service 
'prefijo'     : 'e-sys0/',                             # prefijo  a  todos los topics
...
'Avisos'      :[[ 600,'Los niveles de C O 2 superan ', 'Los niveles de C O 2 estan por debajo de '],  # lista ordenada por CO2 de los avisos
                [1200,'Atencion, Los niveles de C O 2 superan ', 'Los niveles de C O 2 estan por debajo de '],  # al superar el valor se envia el mensaje por el topic <topic_pub>/MSG
                [1800,'Alarma,   Los niveles de C O 2 superan ', 'Los niveles de C O 2 estan por debajo de ']],
...
async def pulse(color):  # This demo pulses LED each time a subscribed msg arrives.
    pin = Pin(cfg[color], Pin.OUT)
    pin.value(1)
...
def sub_cb(topic, msg, retain):
    global Run, station, topic_pub, client, cfg
    asyncio.create_task(pulse("LedGreen"))
...
def media (fifo):
    ...
    for x in fifo:    # desviacion standard
        s0+=(x[0]-t0)*(x[0]-t0)
        s1+=(x[1]-t1)*(x[1]-t1)
    s0=(s0/n)**(1/2); s1=(s1/n)**(1/2)
    if running_time()>cfg["T_WARMUP"]*60:   
        estable0=' ' if cfg["estabilidad"]*t0+5>=max(s0,2) else cfg["NOESTABLE"]  # estabilidad 10%
        estable1=' ' if cfg["estabilidad"]*t1+5>=max(s1,2) else cfg["NOESTABLE"]
    else:
        estable0,estable1=cfg["WARMUP"],cfg["WARMUP"]
    return t0,t1,estable0,estable1
...
def sensorReady(sensortype):
    global sensor, fifo, estableCO2,estableTVOC, blibklevel, uart
    co2=0
    tvoc=0
    ready=False
    if sensortype=='CCS811':
        ready=sensor.data_ready()
        if ready:
            co2=sensor.eCO2
            if sensor.tVOC<5000:
                tvoc=sensor.tVOC
    elif sensortype=='MH-Z19B':
        ...
        if uart.any():
            ...
            if  len (z)==9 and (chksum & 0xFF==0xFF) :
                ...
                ready=True
    if ready:
        fifo.append((co2,tvoc))
        if len(fifo)>cfg["NMAX_MEDIDAS"]:
            fifo.pop(0)
        co2,tvoc,estableCO2,estableTVOC=media(fifo)
        ...
        if sensortype=='CCS811':
            if temp>0 and hum>0:  # compensacion temperatura y humedad
                sensor.put_envdata(hum,temp) 
            if running_time() > cfg["T_BASELINE"] and cfg["T_BASELINE"]>0:  # cada 30 minutos correccion de baseline
                h,l=sensor.get_baseline()
                t0=time()
                sensor.put_baseline (h,l)
    return co2,tvoc

async def pausa(x):
    global grafic, Run, GfxSwt, StpSwt,grfstate, oled, write20, write15
    t0=time()
    while (time()-t0)<x:        
        if cfg['touchpad']:
            t=(GfxSwt.read()<cfg['touchlevel'])
            if grfstate==0 and t:
                grfstate=1
                grafic=True
            elif grfstate==1 and not t:
                grfstate=2
            elif grfstate==2 and t:
                grfstate=3
                grafic=False
                ini_oled(oled,write20,write15)
            elif grfstate==3 and not t:
                grfstate=0
        await asyncio.sleep(0.1)
...
def ini_oled(oled, write20,write15):
    global WiFi
    oled.fill(0)
    txt="eCO2      ppm"; write20.text(txt, 0, 12)
    if cfg['sensorCO2']!='MH-Z19B':
        txt="tVOC      ppb"; write20.text(txt, 0, 34)
...
def WiFiSetup():    
    ...
    ssid="e-sys"
    print ('** WIFI SETUP. Connect your terminal to SSID',ssid,', please')
    ...
    print('Connection successful to access point',ssid)
    print(ap.ifconfig())
...
def clockSetup(ss,pw):
    ...
    if s.isconnected():       
        ok=False
        n=5
        print("** Get time from internet",end="")
...
def load_config():
    ...
    if Factory_reset_Pin()==0: # dos lecturas del pin a 0.5 seg
        ...
        if Factory_reset_Pin()==0:
            print ("\n** Factory reset. Default values assumed **")
            with  open('config.json', 'w') as f:
                json.dump(cfg, f)
                f.close()
            print ("*** Release 'Factory Reset' pin ***")
    ...
    try:
        ...
        if abs(cfg1["Version"]-cfg["Version"])<1: # si versiones difieren en menos de 1 => la misma cfguración
            ...
            with  open('config.json', 'w') as f:
                json.dump(cfg, f)
                f.close()
            print ("\n** Configuration file set to default values **")
    except:
        with  open('config.json', 'w') as f:
            json.dump(cfg, f)
            f.close()
        print ("\n** No conf. file. New one with def val.**")
...
async def mainProgram(client):
    ...
    global display, haysensor, i2c, vspi, adc, s11, temp, hum, tvoc, co2, oled, uart, mensaje
    global StpSwt,GfxSwt,sensor, semMQTT,rssi, mqtt_queue
...
# touchpads
    ...
    haysensor=False
    display=False
    if cfg['displaytype']=="I2C" or cfg['sensorCO2']=="CCS811":
        i2c = SoftI2C(scl=Pin(cfg["SCL"],pull=Pin.PULL_UP), sda=Pin(cfg["SDA"],pull=Pin.PULL_UP), freq=100000)   # instancia bus I2C en pin 21 y 22
    if cfg['displaytype']=="I2C":
        await asyncio.sleep(1)
        devicelist=i2c.scan()
        devicelist=i2c.scan()
        print ("I2C detected address device list:", devicelist)
        display  = 60 in devicelist and cfg['display']
        if not 60 in devicelist:
            cfg['displaytype']="SPI"
        haySensor= 90 in devicelist or (cfg['sensorCO2']=="MH-Z19B")
        if display:
            ok=False
            n=5
            while not ok and (n>0):
            ...
            display=(n>0)   
        if not display:
            print("--List of I2C devices :",i2c.scan())
            print("--Error: no OLED display detected")
        if not haySensor:
            print("--No CCS811 CO2 sensor detected, MH-Z19B assumed")
            cfg['sensorCO2']='MH-Z19B'
    if cfg['displaytype'] in ["SPI", "TFT"]:
        if cfg['displaytype']=="TFT":
        ...
        if cfg['sensorCO2']!='MH-Z19B':
            ...
            devicelist=i2c.scan()
            devicelist=i2c.scan()
            haySensor= 90 in devicelist
            if not haySensor:
                print("--Fatal error: no CO2 sensor detected")
                Run=False    
        else:
            haysensor=True              
    ...
    if cfg["TIPOLECTOR"]=="DHT11":
        s11=dht.DHT11(Pin(cfg["TEMPERATURA"]))         # instancia sensor temperatura DHT11
        s11.measure()
        temp=s11.temperature()
        hum=s11.humidity()
        if temp<5 and hum<5: 
            print ("** DHT-22 detected")
            s11=dht.DHT22(Pin(cfg["TEMPERATURA"]))         # instancia sensor temperatura DHT22
            cfg["TIPOLECTOR"]=="DHT22"
        else:
            print ("** DHT-11 detected")
    else:
        s11=dht.DHT22(Pin(cfg["TEMPERATURA"]))         # instancia sensor temperatura DHT22
        s11.measure()
        temp=s11.temperature()
    ...
    print ("\n** Medidor CO2 . Version "+str(cfg["Version"])+"  "+cfg["VersionDate"]+" **")
     ...
     # set CO2 sensor
    if cfg['sensorCO2']=='CCS811':
        ok=False
        n=5
        while not ok and (n>0):
            try:
                sensor = CCS811.CCS811(i2c)                 # instancia sensor CO2
                ok=True
            except OSError as e:
                print ("-- Err connecting to CCS811")
                n-=1
                await asyncio.sleep(1)
        if n==0:
            print ("** FATAL ERROR: unable to connect with CCS811 sensor **")
            Run=False
        else:
            print ("** CS811 connexion OK ")
    else:
    ...
    while Run:
        ...
        try:
            ...
            temp=s11.temperature()
            hum=s11.humidity()
            tfifo.append((temp,hum))
            if len(tfifo)>cfg["NMAX_MEDIDAS"]:
                tfifo.pop(0)
            temp,hum,testable,hestable=media(tfifo)
        except:
            print ("----DHT fail")
            pass         
        if dg & 2:        
            print ("t3:",time()-tx); tx=time()
        
        # lectura CO² y TVOC
        co2,tvoc=sensorReady(cfg['sensorCO2'])
        ...
        if display:
            if cfg['displaytype']!="TFT":
                if grafic:
                    ...
                    if cfg['sensorCO2']!='MH-Z19B':
                        txt="%4d%c"%(tvoc,estableTVOC)
                        write20.text(txt, 48, 34)
                ...
                tft_display(tft,cfg['sensorCO2'],"%2d.%02d.%02d"%(t[2],t[1],t[0]),"%2d:%02d"%(t[3],t[4]),"%5.1f"%temp,"%4d"%hum,"%5d"%co2,"%5d"%tvoc)
        if dg & 2:
            print ("t5:",time()-tx); tx=time()
...
Pin(cfg['LedBlue' ],value=0)
Pin(cfg['LedRed'  ],value=0)
Pin(cfg['LedGreen'],value=0)
...
# datos de configuracion dinámicos
    ...
    config['will'] = (cfg['topic_pub'], '', False, 0)             # mensaje de despedida
